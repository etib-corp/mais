...
Mais runs a platform-driven render loop around a shared engine instance.

## Typical Startup

1. Create the target platform implementation.
2. Construct `mais::Engine` with that platform.
...
The higher-level application should own the loop, while Mais owns rendering and
